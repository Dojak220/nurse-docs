...
\begin{lstlisting}[caption={Trechos da classe \textit{PatientFormController}}, label={lst:patient_form_controller}, escapeinside={(*}{*)}]
...
Inicialmente, na declaração da classe, utilizou-se o padrão sugerido na própria documentação do pacote \textit{mobx} para geração automática de código por meio do pacote de desenvolvimento \textit{mobx\_codegen} \cite{mobx_core_concepts}. Essa geração torna mais simples a utilização dos recursos do \textit{MobX}, como os \textit{\textbf{Observables}} e \textit{\textbf{Actions}} e, por isso, foi utilizada. Adiciona-se à declaração da classe uma extensão à classe abstrata \textit{AddFormController}, responsável por definir métodos comuns a todas as classes de controle de formulários de inserção de dados como, por exemplo, os métodos \textit{saveInfo} e \textit{updateInfo}.

Em seguida, podemos observar três propriedades observáveis, sendo duas delas listas e a última, um objeto do tipo \textbf{\textit{PatientStore}} que, por sua vez, também possui propriedades observáveis. É nessa última que estão as propriedades que guardam os valores sobre o paciente, enviados pelo usuário na interface.

% [..................................................................]

% Cria uma nova instância de Pacient com os dados do paciente armazenados
% no store, então, o envia ao repositório para ser atualizado no banco de
% dados. 

% Cria uma nova instância de Pacient com os dados do paciente inicial e
% os dados do paciente armazenados no store, então, o envia ao
% repositório para ser atualizado no banco de dados.

\subsubsection{Domínio da Aplicação}
\label{cap4:SubSubSec:Dominio}
Nessa camada estão as classes que representam as entidades do domínio-problema da aplicação. Observa-se que essa camada não depende de nenhuma outra e pode ser considerada aquela que vai dirigir as implementações das demais \cite{evans2017domain}. Essas classes são definidas com base no mesmo modelo descrito com mais detalhes na seção \ref{cap4:SubSec:DiagramaClasses}, mas no trecho de código \ref{lst:vaccine_model} a seguir, é possível ver como a classe \textit{Vaccine} (Vacina) é implementada.
...
\begin{lstlisting}[caption={Trecho da implementação da interface \textbf{GenericModel}}, label={lst:generic_model}]
...
%   \item 
% \end{itemize}
% a camada da \textbf{interface do usuário}, que engloba todos os elementos (\textit{widgets}) de interação com o usuários, como campos de texto ou botões; a camada da \textbf{regra de negócio}, responsável por lidar com as interações realizadas pelo usuário por meio da interface, realizando as mudanças de estados necessários; a camada de \textbf{domínio da aplicação}, responsável por modelar os objetos que representam o domínio no qual a aplicação está inserida e que atuam como os valores de estado modificados pelas regras de negócio; a camada de \textbf{repositório}, que realiza a função de receber as informações advindas da  a \textbf{camada de dados}, responsável por armazenar as informações enviadas . A figura \ref{fig:4_1_1_camadas_nurse} apresenta um diagrama das camadas da aplicação Nurse e a relação entre elas.
